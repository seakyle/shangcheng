...
layui.define(["element","jquery"],function(exports){
	...
	if($(".navBar").html() == ''){
		var _this = this;
		$(".navBar").html(navBar(navs.responseJSON)).height($(window).height()-230);
		element.init();  //初始化页面元素
		$(".layui-nav.layui-nav-tree>li")["0"].click();
	...
	Tab.prototype.tabAdd = function(_this){
		...
		// $(".layui-nav .layui-nav-item a").on("click",function(){
			if(_this.find("i.iconfont,i.layui-icon").attr("data-icon") != undefined || _this.find("i.fa") != undefined){
				var title = '';
				if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
					...
					}else if(_this.find("i.fa") != undefined){
						title += "<i class='"+_this.find("i.fa").attr("class")+"'></i>";
					}else{
					...
					//当前窗口内容
					var curmenu = {
						"icon" : _this.find("i.iconfont").attr("data-icon")!=undefined?_this.find("i.fa")!=undefined ? _this.find("i.iconfont").attr("data-icon"):_this.find("i.fa").attr("class") : _this.find("i.layui-icon").attr("data-icon"),
